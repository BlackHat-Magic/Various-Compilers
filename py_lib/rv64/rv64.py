...
class VirtualMachine:
	...
	def __init__(self):
		self.opcodes: tuple[Callable[[np.uint32], None]] = (
			...
			self._raise(ValueError("Invalid opcode 1011011 (custom-2 or RV128) not supported")),
			# 0b10111_11: 48-bit instruction: the sequel
			self._raise(NotImplementedError("Found 48-bit instruction")),
			self._branch,						# 0b11000_11
			self._jalr,							# 0b11001_11
			# 0b11010_11: Vector
			self._raise(ValueError("Invalid opcode 0b1101011 (reserved) not supported")),
			# 0b11011_11: Jump and link
			self._jal,							# 0b11011_11
			# 0b11100_11: SYSTEM?
			self._raise(NotImplementedError()),
			# 0b11101_11: reserved
			self._raise(ValueError("Invalid opcode: 0b1110111 (reserved) not supported")),
			# 0b11110_11: custom-3 or RV128
			self._raise(ValueError("Invalid opcode 0b1111011 (custom-3 or RV128) not implemented")),
	...
	def _integer_register_immediate(self, inst: np.uint32) -> None:
		...
		match funct3:
			...
			case 0b010:	# SLTI
				self.registers[rd] = np.uint64(1 if src.view(np.int64) < np.int64(imm) else 0)
			case 0b011:	# SLTIU
			...
			case 0b101:	# SRLI/SRAI
				...
				if funct6 == 0b010000:	# SRAI
					self.registers[rd] = np.right_shift(src.view(np.int64), shamt).view(np.uint64)
				elif funct6 == 0:		# SRLI
	...
	def _integer_register_immediate32(self, inst: np.uint32) -> None:
		...
		match funct3:
			...
			case 0b001:	# SLLIW
				...
				self.registers[rd] = np.int64(np.int32(src << (imm & 0x1F))).view(np.uint64)
			case 0b101:	# SRLIW/SRAIW
				...
				if funct6 == 0b010000:	# SRAIW
					self.registers[rd] = np.int64(np.right_shift(
						src.view(np.int32),
						shamt
					).view(np.uint32)).view(np.uint64)
				elif funct6 == 0:		# SRLIW
					self.registers[rd] = np.int64(np.int32(src >> shamt)).view(np.uint64)
				else:
					raise ValueError(
	...
	def _integer_register_register(self, inst: np.uint32) -> None:
		...
		rs1		= (inst >> 15) & 0x1F
		rs2		= (inst >> 20) & 0x1F
		funct7	= (inst >> 25) & 0x7F	# 0100000 = Subtract	0000000 = Add		for funct3=0b000
		...
		try:
			match funct3:
				...
				case 0b010:	# SLT
					if funct7:
						raise ValueError
					self.registers[rd] = np.uint64(1 if np.int64(src1) < np.int64(src2) else 0)
				case 0b011:	# SLTU
					if funct7:
						raise ValueError
				...
				case 0b101:	# SRL/SRA
					if funct7 == 0b0000000:		# SRL
						...
						self.registers[rd] = np.right_shift(src1.view(np.int64), (src2 & 0x3F)).view(np.uint64)
					else:
						raise ValueError
				case 0b110:	# OR
					if funct7:
			...
			match funct3:
				case 0b000:	# ADDW/SUBW
					if funct7 == 0b0000000:		# ADDW
						self.registers[rd] = np.int64(np.int32(src1 + src2)).view(np.uint64)
